...
\begin{note} unlike \tcode{std::is_enum}, \tcode{std::reflect::is_enum} operates on meta-object types.\end{note}
...
\item For \tcode{T} reflecting an array, pointer, reference or function type, or a \emph{cv}-qualified type, the string's value is the empty string for \tcode{get_name<T>} and implementation-defined for \tcode{get_display_name<T>}.
...
\begin{itemize}
  \item for \tcode{T} reflecting an alias (\cxxref{basic}), the unqualified name of the aliasing declaration: the identifier introduced by a \grammarterm{type-parameter} or a type name introduced by a \grammarterm{using-declaration}, alias;
  ...
  \item for \tcode{T} reflecting a \cv-unqualified fundamental type other than \tcode{std::nullptr_t}, the name stated in the "Type" column of Table \cxxref{tref:simple.type.specifiers} in (\cxxref{dcl.type.simple});
  ...
  \item for \tcode{T} reflecting a function parameter, its name;
...
\begin{itemdescr}
\pnum
All specializations of these templates shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates an \tcode{ObjectSequence} specialized with \tcode{RecordMember} types that reflect the following subset of direct non-template members of the class reflected by \tcode{T}:
\begin{itemize}
...
\begin{itemdescr}
\pnum
All specializations of \tcode{get_destructor<T>} shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates a \tcode{Destructor} type that reflects the destructor of the class reflected by \tcode{T}.
...
\begin{itemdescr}
\pnum
All specializations of these templates shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates an \tcode{ObjectSequence} specialized with \tcode{Type} types that reflect the following subset of types declared in the class reflected by \tcode{T}:
\begin{itemize}
...
\begin{itemdescr}
\pnum
All specializations of these templates shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates an \tcode{ObjectSequence} specialized with \tcode{Base} types that reflect the following subset of base classes of the class reflected by \tcode{T}:
\begin{itemize}
...
\begin{itemdescr}
\pnum
All specializations of \tcode{get_enumerators<T>} shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates an \tcode{ObjectSequence} specialized with \tcode{Enumerator} types that reflect the enumerators of the enumeration type reflected by \tcode{T}.
...
\begin{itemdescr}
\pnum
      All specializations of this template shall meet the \tcode{TransformationTrait} requirements (\cxxref{meta.rqmts}). The nested type named \tcode{type} designates an \tcode{ObjectSequence} specialized with \tcode{FunctionParameter} types that reflect the parameters of the function reflected by \tcode{T}.
...
\pnum
      ...
      \begin{note} 
      \tcode{get_constructor_t<get_subexpression_t<reflexpr((T(T())))>>}
      is ill-formed because the \grammarterm{functional-type-conv} expression does not perform overload resolution for a constructor (\cxxref{class.copy.elision}).
      \end{note}
       % TODO: remove the above note when rebasing onto C++14.
\end{itemdescr}
\end{std.txt}
...
\pnum
      ...
      named \tcode{type} designates an \tcode{ObjectSequence} specialized
