...
\item for \tcode{T} reflecting a \cv-unqualified type that is
    ...
    \item a fundamental type other than \tcode{std::nullptr_t}, the name stated in the "Type" column of \cxxref{tref:simple.type.specifiers} in (\cxxref{dcl.type.simple});
    \end{itemize}
\item for \tcode{T} reflecting
    ...
    \item a destructor, the \grammarterm{injected-class-name} of its class, prefixed by the character ’\tilde’;
