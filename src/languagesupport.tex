...
template <class T> concept Constant;                  // refines Scope and ScopeMember
...
// \ref{reflect.ops.expr.fcttypeconf} FunctionalTypeConversion operations
...
\begin{itemdescr}
\pnum
\tcode{Variable<T>} is true if and only if \tcode{T} reflects a variable or non-static data member.
...
\begin{itemdescr}
\pnum
\tcode{Scope<T>} is true if and only if \tcode{T} reflects a namespace (including the global namespace), class, enumeration, function or closure type. Any such \tcode{T} that does not reflect the global namespace also satisfies \tcode{ScopeMember}.
...
\begin{itemdescr}
\pnum
\tcode{Constant<T>} is true if and only if \tcode{T} reflects a constant expression ([expr.const]).
...
\begin{itemdescr}
\pnum
\tcode{Expression<T>} is \tcode{true} if and only if \tcode{T} reflects an expression [expr].
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{ParenthesizedExpression<T>} is \tcode{true} if and only if \tcode{T} reflects a parenthesized expression [expr.prim.paren].
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{FunctionCallExpression<T>} is \tcode{true} if and only if \tcode{T} reflects a \grammarterm{function-call-expression} [expr.call].
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{FunctionalTypeConversion<T>} is \tcode{true} if and only if \tcode{T} reflects a \grammarterm{functional-type-conv-expression} [expr.type.conv].
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{SpecialMemberFunction<T>} is \tcode{true} if and only if \tcode{T} reflects a special member function [special].
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{Operator<T>} is \tcode{true} if and only if \tcode{T} reflects an operator function [over.oper] or a conversion function [class.conv.fct]. \begin{note} Some types that satisfy \tcode{Operator} also satisfy \tcode{MemberFunction} or \tcode{SpecialMemberFunction}. \end{note}
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
\tcode{ConversionOperator<T>} is \tcode{true} if and only if \tcode{T} reflects a conversion function [class.conv.fct].
\end{itemdescr}
\end{std.txt}
...
\begin{example}\begin{codeblock}struct X {};
X x;
using x_t = get_type_t<reflexpr(x)>;\end{codeblock}
While \tcode{get_type_t} is specified to be a \tcode{Type}, \tcode{x_t} will even satisfy \tcode{Class}. \end{example}
...
\begin{itemdescr}
\pnum
All specializations of these templates shall meet the \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type named \tcode{type} is an alias to an \tcode{ObjectSequence} specialized with \tcode{RecordMember} types that reflect the following subset of non-template members of the class reflected by \tcode{T}:
\begin{itemize}
...
\begin{itemdescr}
\pnum
All specializations of these templates shall meet the \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type named \tcode{type} is an alias to an \tcode{ObjectSequence} specialized with \tcode{RecordMember} types that reflect the following subset of function members of the class reflected by \tcode{T}:
\begin{itemize}
   \item for \tcode{get_constructors}, all constructors.
   \item for \tcode{get_destructor}, the destructor;
   \item for \tcode{get_operators}, all conversion functions [class.conv.fct] and operator functions [over.oper].
\end{itemize}
...
requirements ([meta.rqmts]). If \tcode{T} reflects a parameter with a default
...
\begin{itemdescr}
\pnum
      All specializations of this template shall meet the \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type named \tcode{type} is an alias to an \tcode{ObjectSequence} specialized with \tcode{FunctionParameter} types that reflect the parameters of the function reflected by \tcode{T}.
      If that function's \grammarterm{parameter-declaration-clause} [dcl.fct] terminates with an ellipsis, the \tcode{ObjectSequence} does not contain any additional elements reflecting that. The \tcode{is_vararg_v<Callable>} trait can be used to determine if the terminating ellipsis is in its parameter list.
\end{itemdescr}
...
\pnum
      ...
      \tcode{UnaryTypeTrait} requirements ([meta.rqmts]).  If their template
      parameter reflects an entity with an ellipsis terminating the
      \grammarterm{parameter-declaration-clause} [dcl.fct] (for
      ...
      [dcl.inline] (for \tcode{is_inline}), or as deleted (for
...
\begin{itemdescr}
\pnum
      All specializations of \tcode{get_subexpression<T>} shall meet the \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type named \tcode{type} is the \tcode{Expression} type reflecting the expression \tcode{E} of the parenthesized expression \tcode{(E)} reflected by \tcode{T}. 
\end{itemdescr}
\end{std.txt}
...
\begin{itemdescr}
\pnum
      All specializations of \tcode{get_callable<T>} shall meet the \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type named \tcode{type} is the \tcode{Callable} type reflecting the function invoked by the \grammarterm{function-call-expression} which is reflected by \tcode{T}.
\end{itemdescr}
\end{std.txt}
...
\pnum
      ...
      \tcode{UnaryTypeTrait} requirements ([meta.rqmts]). If the template
...
\pnum
      All specializations of this template shall meet the
      \tcode{TransformationTrait} requirements ([meta.rqmts]). The nested type
