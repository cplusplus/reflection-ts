...
\pnum
In C++ [expr.prim.lambda.capture], apply the following change to paragraph \cxxref{expr.prim.lambda.capture.paragraph1}:
\begin{std.txt}
...
\pnum
In C++ [expr.prim.lambda.capture], apply the following change to paragraph \cxxref{expr.prim.lambda.capture.paragraph2}:
\begin{std.txt}
...
In C++ [expr.const], insert a new bullet \cxxref{expr.const.bullet1}, adjusting the subsequent bullet numbers:
...
\begin{itemize}
\setcounter{Bullets1}{4}
  \item \added{a \grammarterm{reflexpr-operand},}
\end{itemize}
\end{std.txt}
