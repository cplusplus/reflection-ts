...
evaluated if the effect of any enclosing \tcode{typeid} expressions (8.5.1.8)
\added{or use of a \grammarterm{reflexpr-specifier} (10.1.7.6)} were ignored,
the entity is said to be \emph{implicitly captured} by each intervening
\grammarterm{lambda-expression} with an associated \grammarterm{capture-default}
...
a \grammarterm{lambda-expression} that is an odr-use (6.2) of an entity captured
by copy\added{, as well as every use of an entity captured by copy in a
\grammarterm{reflexpr-operand}, }is transformed into an access to the
