...
\begin{std.txt}
\begin{itemize}
\item names the entity in a potentially-evaluated expression (3.2) where the enclosing full-expression depends on a generic lambda parameter declared within the reaching scope of the \grammarterm{lambda-expression}\added{ where, for the process of this determination, \tcode{reflexpr} operands are not considered to be unevaluated operands}.
\end{itemize}
\end{std.txt}
