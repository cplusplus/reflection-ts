...
\begin{std.txt}\color{addclr}
    ...
    \begin{itemize}
      ...
      \item \tcode{A} is a class nested in \tcode{B} (\cxxref{class.nest} [class.nest]),
...
(\cxxref{basic.scope.proto}) and the entity is neither captured nor a function
...
unevaluated operand (expr.context).  If the \grammarterm{reflexpr-operand}
designates both an alias and a class name, the type represented by the
\grammarterm{reflexpr-specifier} reflects the alias and satisfies \tcode{Alias}.
