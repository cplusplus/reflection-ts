...
\added{\nontermdef{reflexpr-operand}}\br
  \added{\terminal{::}}\br
  \added{type-id}\br
  \added{nested-name-specifier\opt identifier}\br
  \added{nested-name-specifier\opt simple-template-id}\br
\end{bnf}
...
\begin{std.txt}\color{addclr}
    ...
    \begin{itemize}
      ...
      \item \tcode{A} is not the global namespace and \tcode{B} is an enclosing namespace of \tcode{A}, or
...
For the operand \tcode{::}, the type specified by the \tcode{reflexpr-specifier}
satisfies \tcode{reflect::GlobalScope}.  For an operand of the form
\emph{identifier} where \emph{identifier} is a template \emph{type-parameter},
the type satisfies both \tcode{reflect::Type} and \tcode{reflect::Alias}.
...
\added{value}       & \added{the name of a variable or structured binding}                            & \added{\tcode{reflect::Variable}}        \\
            ...
            &                                                                         & \added{and \tcode{reflect::Constant}}    \\
\bottomline
\end{floattable}
...
scope (6.3.3) or function prototype scope (6.3.4), the program is ill-formed.
If the \grammarterm{reflexpr-operand} designates a class member, the type
...
designates an variable or a data member, it is an unevaluated operand
(expr.context).  If the \grammarterm{reflexpr-operand} designates both an alias
and a class name, the type represented by the \grammarterm{reflexpr-specifier}
reflects the alias and satisfies \tcode{Alias}.
