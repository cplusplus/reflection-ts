...
\rSec1[diff.iso]{\Cpp{} extensions for Concepts with Reflection and {\cppstddocno}}

In {\cppstddocno} [diff.iso], modify the first paragraph:

\begin{std.txt}
This subclause lists the differences between C++ with \added{Reflection and} Concepts and {\cppstddocno} by the chapters of this document.
\end{std.txt}
...
added to enable the definition of concepts (\cxxref{dcl.spec.concept}).
\added{The \tcode{reflexpr} keyword is added to introduce meta-data through a \grammarterm{reflexpr-specifier}.}
\effect
Valid {\cppstddocno} code using \tcode{concept}\removed{ or}\added{,} \tcode{requires}\added{, or \tcode{reflexpr}}
